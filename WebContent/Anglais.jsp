...
<%@ page import="ws.Gestion"%>
<%@ page import="ws.GestionService"%>
<%@ page import="ws.GestionServiceLocator"%>
<%@ page import="beans.*"%>
...
try {
	...
	Livre[] livres = port.chercherLivreCategorie(categorie);
	request.getSession().setAttribute("livre", livres);
	for (int i=0; i < livres.length; i++ ){%>
<div>
	<table class="li" width="80%">			
		<tr>
  			<td class="li"><img src="img/<%=livres[i].getImage() %>" /></td>
            <td class="li"><%=livres[i].getNom() %> <br> <%=livres[i].getAuteur() %></td>               
     		<td class="li">
     			<table>
     				<tr>
     					<td class="hh"><a href="./Panier?nom=<%=livres[i].getId() %>"><button id="registerNew" >Ajouter au Panier</button></a></td>
     				</tr>
     			</table>
     		</td>
			<%}
